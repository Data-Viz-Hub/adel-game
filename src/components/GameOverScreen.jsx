...
export default function GameOverScreen({ state, onRestart }) {
  ...
  const grade = score >= 90 ? { label: 'A', color: C.BLUE }
    ...
    : score >= 55 ? { label: 'C', color: C.WARNING }
    : { label: 'F', color: C.ERROR };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 24, textAlign: 'left' }}>
          {LAYERS.map(l => {
            ...
            return (
              <div key={l.key} style={{
                ...
                padding: '8px 12px',
                background: C.CARD, border: `1px solid ${met ? C.BLUE + '55' : `${C.ERROR}33`}`,
                borderRadius: 7,
              }}>
                <span style={{ fontSize: 14, color: met ? C.BLUE : C.ERROR, flexShrink: 0 }}>
                  {met ? '✓' : '✗'}
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                    <span style={{ color: C.TEXT }}>{l.name}</span>
                    <span style={{ color: met ? C.BLUE : C.ERROR, fontFamily: "'DM Mono', monospace" }}>
                      {pct}% / {l.target}%
                    </span>
                  </div>
          ...
          <div style={{
            padding: '8px 12px',
            background: C.CARD,
            border: `1px solid ${lawsEnacted >= 3 ? C.BLUE + '55' : `${C.ERROR}33`}`,
            borderRadius: 7, fontSize: 12, color: C.TEXT,
            display: 'flex', justifyContent: 'space-between',
          }}>
            <span>Laws enacted</span>
            <span style={{
              color: lawsEnacted >= 3 ? C.BLUE : C.ERROR,
